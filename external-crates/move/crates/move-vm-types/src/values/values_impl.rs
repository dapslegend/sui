...
/// making it possible to be shared by references and used across threads safely.
#[derive(Debug, Clone)]
enum Container {
    ...
    Variant(Arc<Mutex<(VariantTag, Vec<ValueImpl>)>>),
}
...
impl Container {
    fn copy_value(&self) -> PartialVMResult<Self> {
        ...
        Ok(match self {
            ...
            Self::Variant(r) => {
                let (tag, values) = &*r.lock().unwrap();
                let values = values
                    .iter()
                    .map(|v| v.copy_value())
                    .collect::<PartialVMResult<_>>()?;
                Self::Variant(Arc::new(Mutex::new((*tag, values))))
            }
    ...
    fn copy_by_ref(&self) -> Self {
        match self {
            ...
            Self::Variant(r) => Self::Variant(Arc::clone(&r)),
        }
    }
}
    ...
    fn equals(&self, other: &Self) -> PartialVMResult<bool> {
        ...
        let res = match (
            ...
            (Struct(r1), Variant(r2)) | (Locals(r1), Variant(r2)) => {
                r1.lock().unwrap()[self.idx].equals(&r2.lock().unwrap().1[other.idx])?
            }
            (Variant(r1), Vec(r2)) | (Variant(r1), Struct(r2)) | (Variant(r1), Locals(r2)) => {
                r1.lock().unwrap().1[self.idx].equals(&r2.lock().unwrap()[other.idx])?
            }
            (Variant(r1), Variant(r2)) => {
                r1.lock().unwrap().1[self.idx].equals(&r2.lock().unwrap().1[other.idx])?
            }
...
impl Value {
    ...
    pub fn variant(s: Variant) -> Self {
        Self(ValueImpl::Container(Container::Variant(Arc::new(
            Mutex::new((s.tag, s.fields)),
        ))))
    }
        ...
        Some(match (layout, self) {
            ...
            (L::Vector(inner_layout), c) => A::MoveValue::Vector(match c {
                ...
                Container::Vec(r) => r
                    .borrow()
                    .iter()
                    .map(|v| v.as_annotated_move_value(inner_layout.as_ref()))
                    .collect::<Option<_>>()?,
